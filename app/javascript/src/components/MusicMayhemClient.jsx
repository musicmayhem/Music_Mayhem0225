...
class MusicMayhemClient extends React.Component {
      ...
      if (res) {
        this.props.dispatch(
          checkPlayerPresent({
            ...
            player: { account_id: res.account.id },
          }),
        );
        this.props.dispatch(
          actions.getGameData({
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      (!this.props.game.rounds || this.state.allowJoin) &&
      this.props.player &&
      ...
      Swal.fire({
        ...
        nextProps.dispatch(
          postRequest("games/player_playlist", {
            values: {
              game: {
                code: nextProps.match.params.game_code,
    ...
    if (
      ...
      nextProps.game.game &&
      nextProps.game &&
      nextProps.game.current_profile
    ) {
      this.loadGameData(nextProps.game);
      this.loadPlayerGuessData(nextProps.game);
      this.setState({
        leaderboardTime:
          nextProps.game.current_profile.leaderboard_display_time,
      });
      // pusher.unsubscribe('games_' + nextProps.game.game.id)
      this.getSongPlayCount();
      if (
        nextProps.game.state == "Active Song" ||
        nextProps.game.state == "Song Loaded"
      ) {
        this.setState({
          currentSong: "default",
          beginGame: true,
      ...
      if (
        ...
        nextProps.game &&
        nextProps.game.game &&
        nextProps.game.game.session_id
      ) {
        this.props.dispatch(
          postRequest("player/rewards", {
            type: REWARDS,
            values: { session_id: nextProps.game.game.session_id },
          }),
        );
      }
      if (nextProps.game.game.state == "Game Over" && !this.state.gameOver)
  ...
  playerAlert(nextProps) {
    ...
    if (
      player_song_play_count == 11 &&
      nextProps.player.introRedeemed == false &&
      !this.state.introRedeemed
    )
      this.showIntroAlert();
    ...
    if (player_song_play_count == 2)
      this.noticeAlert(
        "💡 TIP: If the game gets stuck, try refreshing your screen, or ask your host for help",
      );
    if (player_song_play_count == 3)
      this.noticeAlert("⏱ TIP: Faster guesses earn more points");
    ...
    if (player_song_play_count == 9)
      this.noticeAlert(
        "🎤 TIP: Not great at texting? Try Voice To Text (so only your phone hears you)",
      );
    ...
    if (player_song_play_count == 11)
      this.noticeAlert(
        "🔊 Hey-o! You've already racked up 11 songs! You better slow down or you might win something!",
      );
  }
  ...
  showIntroAlert() {
    this.props
      .dispatch(gamePlayers({ game: { code: this.props.game.game.code } }))
      .then((res) => {
          ...
          Swal.fire({
            ...
            preConfirm: (playerName) => {
              this.props
                .dispatch(
                  checkPlayerName({
                    player: { name: gamePlayers[playerName] },
                  }),
                )
                .then((res) => {
                  if (
  ...
  getSongPlayCount() {
    this.props.dispatch(
      makeRequest("player/songs_played", { type: PLAYER_SONGS_PLAYED }),
    );
  }
      ...
      (data.account_id == "all" ||
        (this.props.player &&
          ...
          (this.props.player.player.account_id == data.account_id ||
            (Array.isArray(data.account_id) &&
              data.account_id.includes(this.props.player.player.account_id)))))
    ) {
      this.animateReward(data);
      if (
        data.auto_ticket &&
        !["score11", "score111"].includes(data.auto_ticket.status)
      ) {
        setTimeout(() => {
          this.ticketAlert(
            "🏆 +" +
        ...
        this.ticketAlert(
          "⭐️ +" + data.auto_ticket.ticket_count + " tickets for score 111",
        );
      }
    }
  ...
  animateReward(data) {
    if (document.getElementById(data.reward)) {
      ...
      if (data.account_id == "all" || Array.isArray(data.account_id))
        this.ticketAlert(
          "🏆 + Hey! You just got " + data.count + " " + data.reward + "!",
        );
      setTimeout(() => {
        ...
        switch (data.type) {
          ...
          case "showing_leaderboard":
            ...
            if (
              this.props.game &&
              this.props.game.game &&
              this.props.game.game.reset_round
            ) {
              this.props.dispatch(
                updatePlayerPusherRequest({
          ...
          case "song_ended":
            this.props.dispatch(
              updatePlayerPusherRequest({ currentSong: "", state: data.type }),
            );
            break;
          case "game_ended":
          ...
          case "set_playlist_by_player":
            if (
              this.props.player.player &&
              this.props.player.player.id &&
              this.props.player.player.id == data.player
            )
              this.selectGamePlaylists();
            else if (
              this.props.player.player &&
          ...
          case "tile_reveal":
            this.setState({
              visibleTitle: data.data.visible_title,
              visibleArtist: data.data.visible_artist,
            });
            break;
          case "guess_end":
              ...
              if (result.value) {
                this.props.dispatch(
                  postRequest("games/pusher_update", {
                    values: {
                      game: {
                        code: this.props.game.game.code,
          ...
          case "game_winner":
            if (
              ...
              setTimeout(() => {
                this.ticketAlert(
                  "🏆 +1 Prize! YOU WON THE GAME! Redeem from your Home screen!",
                );
              }, 18000);
            }
            break;
  ...
  inputFieldAnimation() {
    if (document.getElementById("guessInput")) {
      ...
      setTimeout(() => {
        if (document.getElementById("guessInput"))
          document.getElementById("guessInput").removeAttribute("style");
      }, 3000);
    }
  }
  ...
  render() {
    ...
    const points =
      roundTotalScore || (this.props.game && this.props.game.points);
    const pusherDataCondition =
      beginGame && player && player.player && player.playerPusherData;
    const checkGameState = state == "Active Song" || state == "Song Loaded";
    const guessScreenConditions =
      pusherDataCondition && player.playerPusherData.state == "song_loaded";
    const reloadedGuessScreenConditions =
      !beginGame &&
      player &&
      player.player &&
      this.props.game &&
      ...
      <div>
        {!triviaMode && !mayhemMatesMode && (
          <div>
              ...
              <div>
                <PlayerGuessScreen
                  ...
                  pusherCurrentSongCount={
                    pusherCurrentSongCount
                      ? pusherCurrentSongCount
                      : this.props.game.game.song_of_songs_round_count
                  }
                  song={currentSong}
                  ...
                  setRoundTotalScore={(x) =>
                    this.setState({ roundTotalScore: x })
                  }
            ...
            {pusherDataCondition &&
              player.playerPusherData.state == "showing_leaderboard" && (
                <div>
                  <PlayerSongEnd
                    session={session}
                    gameCode={this.props.match.params.game_code}
            ...
            {pusherDataCondition &&
              player.playerPusherData.state == "show_ad_camp" && (
                <div>
            ...
            {pusherDataCondition &&
              player.playerPusherData.state == "game_ended" && (
                <PlayerGameEnd
                  ...
                  leaderboard={player.playerPusherData.leaderboard}
                  gameOverTime={
                    this.props.game.rounds.settings.game_over_display_time
                  }
                />
              )}
        ...
        {triviaMode && (
          <StandardTriviaClient
            {...this.props}
            answerState={this.state.answerUpdated}
          />
        )}
        {mayhemMatesMode && (
          <MayhemMatesClient

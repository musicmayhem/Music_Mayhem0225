...
import {
  ...
  getGameData,
  clearState,
  sendVolumePusherRequest,
} from "../actions/hostGameActions";
...
const gameTimes = START_GAME_IN.map((game_time, index) => (
  <option key={index}> {game_time.time} </option>
));
...
class GameConfiguration extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    this.props.checkUserIsLogin().then((res) => {
      if (!res) {
        ...
        this.props.makeRequest("games/get_game_profiles", {
          type: GET_GAME_PROFILES_SUCCESS,
        });
        this.props.makeRequest("games/get_open_series_list", {
          type: GET_OPEN_SERIES_LIST_SUCCESS,
        });
        this.props.makeRequest("games/get_open_session_list", {
          type: GET_OPEN_SESSION_LIST_SUCCESS,
        });
        this.props.makeRequest("games/generate_playlist", {
          type: GET_PLAYLIST_AND_AD_CAMP_SUCCESS,
        });
        this.props.dispatch(
          getGameData({ game: { code: this.props.match.params.game_code } }),
        );
        this.setState({ accountRole: res.account.role });
        if (res.account.role == "host")
          this.setState({ hostId: res.account.id });
      }
    });
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (!this.props.game.game && nextProps.game.game) {
      ...
      if (this.props.game.profile && nextProps.game.profile) {
        const profile = nextProps.game.profile.filter(
          (x) => x.id == nextProps.game.game.profile_id,
        )[0];
      ...
      if (
        ...
        this.setState({
          showBackgroundMusicControl: true,
          showBackgroundMusicPlaylist: true,
        });
      if (
        nextProps.game &&
        nextProps.game.game &&
    ...
    if (localStorage["game_config_updated"]) {
      if (this.state.accountRole == "host") {
        if (this._config_page) {
          nextProps.history.push(
            "/remote/" + this.props.match.params.game_code,
          );
        ...
        if (
          ...
          if (
            ...
            nextProps.history.push(
              "/player/" + this.props.match.params.game_code,
            );
            localStorage.removeItem("game_config_updated");
          }
        }
      ...
      if (
        nextProps.game.gameDataRecieved &&
        nextProps.game.rounds &&
        nextProps.game.rounds.settings &&
        this._runOnce
      ) {
        arr.map((x) =>
          ["song_count", "playlist_id", "background_music_playlist"].includes(x)
            ? this.props.changeFieldValue(
                "game." + x,
                nextProps.game.rounds.settings[x],
              )
            : this.props.changeFieldValue("game." + x, nextProps.game.game[x]),
        );
        if (nextProps.game.rounds.settings["playlist_id"])
          this.setState({
            selectedPlaylist: nextProps.game.rounds.settings["playlist_id"],
          });
        this._runOnce = false;
      }
    }
    ...
    if (
      nextProps.game &&
      nextProps.game.game &&
      nextProps.game.game.account_id != this.state.hostId
    )
    ...
    if (
      nextProps.game &&
      nextProps.game.songsData &&
      nextProps.game.songsData.length > 0
    ) {
      this.setState({ songsData: nextProps.game.songsData });
      if (nextProps.game.songsData != this.state.songsData)
        this._skipSong = true;
    }
  }
  ...
  updatedGameValues = (values) => {
    var playlist =
      ...
      this.props.game.playlist.filter(
        (p) => p[0] == values["game"]["playlist_id"],
      );
    if (values["game"]["trivia_url"]) this.startTrivia();
    else if (this.state.mayhemMatesMode) this.startMayhemMates();
    else if (
      this.state.accountRole == "host" &&
      playlist[0] &&
      values["game"]["song_count"] > playlist[0][2]
    ) {
      Swal.fire({
        type: "warning",
      ...
      this.state.accountRole == "host" &&
      (values.game.campaign_id == "0" ||
        values.game.campaign_id == null ||
        this.venue_name === "")
    ) {
      this.campaign_id = 0;
      Swal.fire({
      ...
      if (this.state.sessionOption == "new") {
        ...
        if (!this.props.game.current_session)
          values["game"]["session"] = this.state.modalData;
        else values["game"]["session"] = this.props.game.session;
      }
      if (this.state.sessionOption == "existing")
        values["game"]["session_id"] = this.state.sessionId;
      values["game"]["profile_data"] =
      ...
      if (values["game"]["timer"] == "30 seconds")
        values["game"]["timer"] = "0.5 minute";
      ...
      values["game"]["profile_id"] =
        (this.state.profileData && this.state.profileData.id) ||
        defaultProfileData.id;
      if (this.state.remoteHostId)
        values["game"]["remote_host_id"] = this.state.remoteHostId;
    ...
    if (p !== "SELECT SERIES") {
      switch (p) {
        ...
        default:
          ...
          this.props.postRequest("games/get_series_detail", {
  ...
  updateGameConfigurationFromProfile = (p) => {
    ...
    if (p == "Trivia Mode") {
      if (
        ...
        window.history.pushState(
          "object or string",
          "Title",
          "config/" + this.props.match.params.game_code + "?Continue",
        );
      } else
        Swal.fire({
          type: "warning",
    ...
    if (p !== "SELECT PROFILE") {
      ...
      this.setState({
        profileData: data,
        showBackgroundMusicPlaylist: data["background_music"],
      });
      arr.map((x) => this.props.changeFieldValue("game." + x, data[x]));
      if (data["playlist_id"] != null) {
        this.setState({
          selectedPlaylist: data["playlist_id"],
          filteredSongCount: null,
          playlistEras: [],
          playlistGenres: [],
        });
  ...
  modalValues = (values) => {
    values = {
      name:
        values ||
        this.props.dispatch(
          change("updatedGameForm", "session.name", this.venue_name),
        ).payload,
  ...
  sendPlaylistPusherToPlayer(player) {
    this.props.postRequest("player/set_playlist", {
  ...
  getCampaign = (campaign_id) => {
    ...
    if (campaign_id === "0") {
      camp_venue_name =
        ...
        this.props.game.campaign.find((x) => x[0] == campaign_id)[1];
    } else {
      camp_venue_name =
        this.props.game &&
        this.props.game.campaign &&
    ...
    var name = this.props.dispatch(
      change("updatedGameForm", "session.name", camp_venue_name),
    ).payload;
    ...
    if (
      ...
      camp_venue_name &&
      this.props.game.sessionList.map(
        (x) => x.name.substr(0, length) === camp_venue_name.toLowerCase(),
      ) &&
      this.props.game.sessionList
    ...
    if (
      this.props.profile == "Trivia Mode" ||
      this.props.profile == "Mayhem Mates Mode"
    )
  ...
  getSongData() {
    if (
      !this.state.reviewPlaylist &&
      (!this.state.songsData || this.state.configChange)
    ) {
      if (
        (this.state.filteredSongCount &&
          parseInt(this.state.filteredSongCount) <
            parseInt(this.props.song_count)) ||
        !parseInt(this.props.song_count) ||
        this.props.song_count == ""
      ) {
        Swal.fire({
          type: "warning",
          ...
          .instantRequest("games/get_song_data", {
            values: {
              ...
              game: {
                ...
                random_play: this.props.random_play,
              },
        ...
        postRequest(
          "games/skip_song_data",
          {
            ...
            values: {
              game: {
                code: this.props.match.params.game_code,
                song_skip_count: this.state.songSkipCount,
              },
              songId: song_id,
            },
          },
  ...
  render() {
    ...
    let seriesData =
      this.props.game.seriesList &&
      this.props.game.seriesList.length > 0 &&
      this.props.game.seriesList.map((x) => x.name);
    if (seriesData)
      seriesOptions = SERIES_OPTIONS.concat(seriesData).map((series, index) => (
        <option key={index}> {series} </option>
      ));
    else
      seriesOptions = SERIES_OPTIONS.map((series, index) => (
        <option key={index}> {series} </option>
      ));
    ...
    if (
      this.props.game &&
      this.props.game.game &&
      this.props.game.game.current_song_count &&
      this.props.song_count
    ) {
      totalSongCount =
        parseInt(this.props.game.game.current_song_count.slice(4, 7)) +
        parseInt(this.props.song_count) -
        1;
    }
    if (
      ...
      this.props.game.profile.filter((x) => x.name === "SELECT PROFILE")
        .length === 0
    )
    ...
    return (
      <div>
                  ...
                  <Col sm="12" md={{ size: 6, offset: 3 }}>
                    <div className="py-3">
                      <Row center="xs">
                        <Col
                          x={4}
                          style={{ color: "#ffca27", marginBottom: "15px" }}
                        >
                          <a
                            style={{ float: "left" }}
                            onClick={() => {
                          ...
                          {!standardTriviaMode &&
                            ...
                            !current_session && (
                              <Field
                                ...
                                onChange={(p) => (
                                  this.getCampaign(p.target.value),
                                  (this.campaign_id = p.target.value)
                                )}
                                options={
                                  campaign
                                    ? campaign.map((p) => (
                            ...
                            <div>
                              {accountRole == "host" && (
                                <Field
                                  ...
                                  onChange={(p) => {
                                    this.updateGameConfigurationFromProfile(
                                      p.target.value,
                                    );
                                    this.setState({
                                      songsData: null,
                                    ...
                                    >
                                      ...
                                      >
                                        <PlaylistFilter
                                          ...
                                          playlistGenres={
                                            this.state.playlistGenres
                                          }
                                          filteredSongCount={filteredSongCount}
                                          filterPlaylistModal={(x) => {
                                            this.setState({
                                              filterPlaylistModal: x,
                                            });
                                          }}
                                          ...
                                          setGenres={(genres) => {
                                            this.setState({
                                              playlistGenres: genres,
                                              configChange: true,
                                              songsData: null,
                                              songSkipCount: 0,
                                            });
                                          }}
                                          setFitererdSongCount={(
                                            filteredSongCount,
                                          ) => {
                                            if (
                                              parseInt(filteredSongCount) &&
                                              parseInt(this.props.song_count) >
                                                parseInt(filteredSongCount)
                                            )
                                              this.props.changeFieldValue(
                            ...
                            <div>
                              <Collapse isOpen={this.state.collapse}>
                                          ...
                                          <Row start="xs">
                                            ...
                                            >
                                              <a
                                                ...
                                                onClick={() =>
                                                  this.sendPlaylistPusherToPlayer(
                                                    x,
                                                  )
                                                }
                                              >
                                ...
                                {!current_session && accountRole == "host" && (
                                  ...
                                  >
                                    ...
                                    <div
                                      ...
                                      onClick={() => {
                                        this.setState({
                                          sessionOption: "existing",
                                          openSessionList: true,
                                        });
                                      }}
                                    >
                                      <div>
                                ...
                                {current_session && (
                                  ...
                                  >
                                    Session:{" "}
                                    <span style={{ color: "#ffca27" }}>
                                      {" "}
                                      &ldquo; {current_session} &rdquo;{" "}
                                    </span>
                                  </h5>
                                )}
                                {!current_session &&
                                ...
                                {!current_session &&
                                  accountRole == "host" &&
                                  this.state.sessionOption === "existing" && (
                                    <Field
                                      id="sessionField"
                              ...
                              {showBackgroundMusicControl && (
                                <BackgroundMusicControl
                                  volumeChange={(t) => this.volumeButton(t)}
                                />
                              )}
                            </div>
                          )}
        ...
        {gameOverScreen && (
          <GameOverScreen
            resetGameFromOverScreen={(val) => this.resetGame(val)}
            configScreen
          />
        )}
      </div>
...
export default connect((state) => {
  ...
  const background_music_playlist = selector(
    state,
    "game.background_music_playlist",
  );

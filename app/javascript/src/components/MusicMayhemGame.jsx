...
class MusicMayhemGame extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      game.songSkipped &&
      game.rounds &&
      game.state == "Starting Game" &&
      !this.state.songSkipped
    ) {
      ...
      this.props.dispatch(
        actions.startingRoundServer({ round: nextProps.game.rounds }),
      );
    }
    if (
      nextProps.auth.currentAccount &&
      game.game &&
      nextProps.auth.currentAccount.id != game.game.account_id
    ) {
      const game_code = this.props.match.params.game_code;
      ...
      if (game.already_played_song_ids.length == 0) {
        ...
        this.props.dispatch(
          actions.loadSong({
            song: { id: currentSong },
            game: game.game.code,
            song_ids: game.rounds.song_order_ids,
          }),
        );
      } else {
        if (
          game.rounds.song_order_ids.filter(
          ...
          game.rounds.song_order_ids.toString() ==
          game.already_played_song_ids.toString()
        ) {
          this.props.dispatch(
            actions.updateGameRequest({
              state: "Game Over",
    ...
    if (
      ...
      if (game && game.song_order_ids && game.song_order_ids.length == 1)
      ...
      this.props.dispatch(
        actions.loadSong({
          song: { id: loaded_song_id },
          game: game.game.code,
          song_ids: game.song_order_ids,
        }),
      );
    }
    ...
    if (!this.state.scoreboardUrl && game && game.series_name)
      this.setState({
        scoreboardUrl: window.location.origin + "/series/" + game.series_name,
      });
  }
      ...
      channel.bind("game_event", (data) => {
        ...
        switch (data.type) {
          case "set_current_round":
            this.props.dispatch(
              updatePusherRequest({ rounds: data.data, state: data.type }),
            );
            break;
          case "song_loaded":
            this.setState({
          ...
          case "showing_leaderboard":
            ...
            if (
              this.props.game.song_order_ids &&
              this.props.game.song_order_ids.length == 0
            )
              this._lastSongOfGame = true;
          ...
          case "song_ended":
            this.props.dispatch(
              updatePusherRequest({ currentSong: "", state: data.type }),
            );
            break;
          case "game_ended":
            this.setState({
          ...
          case "first_player_added":
            ...
            if (
              ...
              !this.props.game.game.passive_mode
            )
          ...
          case "advance_next_song":
            if (document.getElementById("next-song-btn"))
          ...
          case "open_spin_wheel":
            this.setState({
              ...
              showPpt: false,
              slotMachine: false,
              wheelType: data.data.wheel_type,
              playerNames: data.data.player_names,
            });
            break;
          case "start_spin_wheel":
            if (this.refs.SpinWheel)
              this.refs.SpinWheel.spin(data.data.random_spin);
            setTimeout(() => {
              this.setState({
                spinWheel: false,
                showPpt: false,
          ...
          case "mayhem_mates_started":
            window.location =
              "/mayhem_mates/" + this.props.match.params.game_code;
            break;
          ...
          case "start_next_round":
            this.props.dispatch(
              actions.addNewRound({
                game: { code: this.props.game.game.code },
              }),
            );
            localStorage["game_updated"] = true;
            break;
          ...
          case "campaign_updated":
            if (
              !this.state.guessActive &&
              (data.data.jukebox ||
                (this.props.game.game && this.props.game.game.jukebox_mode))
            )
              this.updateAppliance();
  ...
  render() {
    ...
    return (
      <div className="video-background">
        {slotMachine && (
          <SlotMachine {...this.props} spiffValue={this._spiffValue} />
        )}
        {spinWheel && (
          <SpinWheel
            ref="SpinWheel"
            wheelType={wheelType}
            playerNames={
              playerNames && playerNames.length > 0 ? playerNames : null
            }
            mayhemSpinnerUpdate={this.mayhemSpinnerUpdate}
          />
        )}
        {showPpt && (
          <Advertisement
        ...
        {!demoGame && (
          <div
            style={{ opacity: !(slotMachine || showPpt || spinWheel) ? 1 : 0 }}
          >
            {!beginGame && game.state == "Starting Game" && (
              <HostGameTimer
                ...
                scoreboard={{
                  showScoreboard: game.game.show_scoreboard,
                  scoreboardDuration: scoreboardDuration,
            ...
            {gameScreenBigConditons && (
              <ScreenRouter
                game={{
                  ...
                  songCount: pusherCurrentSongCount
                    ? pusherCurrentSongCount
            ...
            {(beginGame || gameEnded) && game.state == "Game Over" && (
              <FinalLeaderboard gameCode={game.game.code} />
            )}
          </div>
        )}
        {demoGame && (

...
class Example extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(np) {
    ...
    else if (url.includes("/mayhem_mates/"))
  ...
  createHostedGame() {
    ...
    localStorage.removeItem("game_config_updated");
    localStorage.removeItem("game_updated");
    this.props.postRequest("/games", {
      type: CREATE_GAME_FROM_HEADER,
      values: values,
    });
  }
    ...
    return (
      <div style={{ display: this.state.showHeader ? "" : "none" }}>
        ...
        <div
          className={
            this.state.isOpen
              ? "nav-bar-content nav-bar-content__open"
              : "nav-bar-content"
          }
        >
          <div className="nav-bar-content-links">
            {showLiveGiftOption &&
              ...
              this.props.game.game.code && (
                <p>
                  <Link
                    style={{ color: "yellow", marginTop: "-5px" }}
                    to={"/gifting/" + this.props.game.game.code}
                  >
              ...
              <div>
                ...
                {this.props.auth.currentAccount &&
                  ...
                  this.props.auth.currentAccount.role == "host" && (
                    <Link to="/series">Check League Scores</Link>
                  )}
                {this.props.auth.currentAccount &&
                  this.props.auth.currentAccount.role &&
                  this.props.auth.currentAccount.role == "player" && (
                    <Link to="/my_score">My League Standing</Link>
                  )}
            ...
            {this.props.auth.currentAccount &&
              this.props.auth.currentAccount.role &&
              this.props.auth.currentAccount.role == "host" && (
                <Link to="/monitor">Mayhem Monitor</Link>
              )}
            {this.props.auth && !this.props.auth.accountLoggedIn && (

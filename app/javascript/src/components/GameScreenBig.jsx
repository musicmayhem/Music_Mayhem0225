...
export const GameScreenBig = ({
  ...
  const revealInterval = useRef(
    game.time / (game.seq.title.length + game.seq.artist.length),
  );
  ...
  const [visibleSongName, setVisibleSongName] = useState(() =>
    makeTitleHash(game.songName),
  );
  const [visibleArtistName, setVisibleArtistName] = useState(() =>
    makeArtistHash(game.artist),
  );
  ...
  const handleGuessEnd = useCallback(() => {
    ...
    setTimeout(() => {
      onUpdateGameRequest({
        game: { code: game.gameCode, state: "Showing LeaderBoard" },
      });
    }, songPlayTime);
  }, [
    mirror,
  ...
  useEffect(() => {
    ...
    if (player) {
      player.onplaying = () => {
        ...
        revealAllowedAtRef.current =
          Date.now() + (game.letterStartTime || 0) * 1000;
        setIsPlaying(true);
      };
    }
  ...
  useEffect(() => {
    ...
    const startReveal = () => {
      const remainingLetters =
        titleSeqRef.current.length + artistSeqRef.current.length;
    ...
    const startDelayId =
      delayMs > 0
        ? setTimeout(startReveal, delayMs)
  ...
  const renderTiles = (hashString) => {
    ...
    letters.push(
      `<div key=${hashString.length} class='letter-big space-letter-big'></div>`,
    );
  ...
  const makeWords = (strings) => {
    ...
    strings.forEach((s, i) => {
      ...
      if (
        div.firstChild.classList.contains("space-letter-big") &&
        i !== strings.length - 1
      )

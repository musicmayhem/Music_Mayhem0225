...
class Leaderboard extends React.Component {
  state = {
    ...
    displayTime:
      this.props.game && this.props.game.game
        ? this.props.game.rounds.settings.leaderboard_display_time * 1000
        : 5000,
  };
  ...
  sendNextEvent() {
    ...
    if (!this.props.mirror) {
      if (this.props.openSession && this.props.lastSongOfGame) {
        ...
        this.props.postRequest("games/pusher_update", {
          values: {
            game: { code: this.props.game.game.code, status: "gameOver" },
          },
        });
      } else {
        setTimeout(() => {
    ...
    return (
      <div style={{ color: "#fff" }}>
        {lastSongOfGame && (
          <div className="yellow-header">
            <Row>
              ...
              <div className="timer">
                {game.game.open_session &&
                  game.game.automatic_round_advance && (
                    <Timer
                      time={0.5}
                      data={"Next Round will start in"}
                      position="top"
                    />
                  )}
              </div>
        ...
        <Row
          ...
          className={
            this.state.showInfoSong
              ? "animated-song-name animate-song-name"
              : "animated-song-name"
          }
        >
        ...
        <Row center="xs">
          <Col
            ...
            style={{
              margin: "2rem 0",
              padding: "0 2rem",
              borderRight: "1px solid #210345",
              textAlign: "left",
            }}
          >
            {this.state.showListNormal && (
              <ListNormal
                songLeaders={songLeaders}
                player1={player1}
                player11={player11}
                session={openSession}
              />
          ...
          <Col
            xs={12}
            md={6}
            style={{ margin: "2rem 0", padding: "0 2rem", textAlign: "left" }}
          >
            {this.state.showListColored && round_leaderboard && gameLeaders && (
              <ListColored gameLeaders={gameLeaders} session={openSession} />
            )}
          </Col>
        </Row>
